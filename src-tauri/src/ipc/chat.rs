...
pub async fn send_message(
    ...
    match provider.as_str() {
        ...
        "openai" => {
            send_message_openai(&config, model, messages, temperature, top_p, max_tokens, stop_strings).await
        },
        "gemini" => {
            send_message_gemini(&config, model, messages, system_prompt, temperature, top_p, max_tokens, stop_strings).await
        },
        "local_ollama" | "local_lmstudio" => {
            send_message_local(&config, &provider, model, messages, temperature, top_p, max_tokens, stop_strings).await
        },
...
pub async fn stream_message(
    ...
    if enable_web_search.unwrap_or(false) {
        ...
        let search_provider = match active_search {
            "brave" => {
                let brave_key = config.brave_api_key.clone().ok_or("Brave API key not set. Right-click the globe to configure.")?;
                search::SearchProvider::Brave(brave_key)
            },
            "searxng" => {
                let searx_url = config.searxng_url.clone().ok_or("SearXNG URL not set. Right-click the globe to configure.")?;
                search::SearchProvider::SearXNG(searx_url)
            },
            _ => {
                let tavily_key = config.tavily_api_key.clone().ok_or("Tavily API key not set. Click the globe to configure.")?;
    ...
    match provider.as_str() {
        ...
        "openai" => {
            stream_message_openai(&config, model, messages, temperature, top_p, max_tokens, stop_strings, channel, state.abort_flag.clone()).await
        },
        "gemini" => {
            stream_message_gemini(&config, model, messages, system_prompt, temperature, top_p, max_tokens, stop_strings, channel, state.abort_flag.clone()).await
        },
        "local_ollama" | "local_lmstudio" => {
            stream_message_local(&config, &provider, model, messages, temperature, top_p, max_tokens, stop_strings, channel, state.abort_flag.clone()).await
        },
        _ => Err(format!("Unsupported provider: {}", provider))
    }
}
