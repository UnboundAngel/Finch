...
import type { WebSearchResearchEvent } from '@/src/types/chat';

interface SearchStatusProps {
  events: WebSearchResearchEvent[];
  isThinking?: boolean;
}

export const SearchStatus = ({ events }: SearchStatusProps) => {
  ...
  // Start collapsed if already done (loaded from history); expanded while in-progress
  const [isExpanded, setIsExpanded] = useState(!isDone);

  // Auto-collapse as soon as search completes
  useEffect(() => {
    if (isDone) setIsExpanded(false);
  }, [isDone]);
  ...
  return (
    <div className="mb-4">
      <div className="inline-flex flex-col min-w-[300px] max-w-full rounded-2xl border border-white/10 bg-white/5 backdrop-blur-xl shadow-xl overflow-hidden">
        {/* Header */}
        <div
        ...
        >
          <div className="flex items-center gap-3">
            <Binoculars className={cn("h-4 w-4 text-muted-foreground", !isDone && "animate-pulse")} />
